  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const targetPath = data.redirect || (role === "donor" ? "/donor" : role === "hospital" ? "/hospital" : role === "blood-lab" ? "/lab" : role === "admin" ? "/admin" : "/");
        ...
        // DEMO MODE FALLBACK
        console.log("API error, running demo mode fallback.");
        toast.success("Login Successful!");
        localStorage.setItem("role", "donor"); // default for demo mode
        setTimeout(() => {
      ...
      // DEMO MODE FALLBACK ON NETWORK ERROR
      toast.success("Login Successful!");
      localStorage.setItem("role", "donor"); // default for demo mode
      setTimeout(() => {
        navigate("/donor", { replace: true });
